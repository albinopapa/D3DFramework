...
namespace simd
{
	...
	const float4 mTenth = _mm_set_ps1( .1f );
	const float4 mHalf = _mm_set_ps1( .5f );
	const float4 m3Halves = _mm_set_ps1( 1.5f );
...
struct Simd_sp
{
	...
	{}
	// Instantiate SIMD class from __m128 object (aliased as float4)
	...
	// The four immediate numbers passed as template parameters, 
	// correspond to which element gets picked. 
	// Ex, <0,0,0,0> would mean copy the X element to all four lanes
	template<int X, int Y, int Z, int W>Simd_sp &Shuffle()
	{
		vec = _mm_shuffle_ps( vec, vec, _MM_SHUFFLE( W, Z, Y, X ) );
		return *this;
	}
	// Anonymous shuffle function template, shuffles self with A.  
	// Modifies self
	...
	__declspec ( property ( get = SplatX, put = SetXXXX ) ) Simd_sp xxxx;
	void SetXXXX( const Simd_sp &Src ) { *this = Simd_sp( Src ).SplatX(); }
	// Splat functions all modify self. They use the anonymous Shuffle 
	// functions above.
	...
	// Unaligned store using Simd_sp* as parameter
	void Store( Simd_sp *pSimd )
	{
		Store( reinterpret_cast< float* >( pSimd ) );
	}
...
// Function is used to roll A by Count.  
...
Simd_sp _mm_rolll_ps( const Simd_sp &A )
{
	...
	switch ( count )
	{
		case 0:
			...
			break;
		case 1:
			return Simd_sp( A ).Shuffle<1, 2, 3, 0>();
			break;
		case 2:
			return Simd_sp( A ).Shuffle<2, 3, 0, 1>();
			break;
		case 3:
			return Simd_sp( A ).Shuffle<3, 0, 1, 2>();
	}
}
...
inline Vec2p operator/( const Simd_sp &A, const Vec2p &B)
{
	return Vec2p( A / B.x, A / B.y );
}
inline Simd_sp DotProduct( const Vec2p &A, const Vec2p &B )
{
	const Simd_sp result = ( ( A.x * B.x ) + ( A.y * B.y ) );
	return result;
}
inline Simd_sp SqMagnitude( const Vec2p &A )
{
	return DotProduct( A, A );
}
inline Simd_sp Magnitude( const Vec2p &A )
{
	return Sqrt( SqMagnitude( A ) );
}
inline Vec2p Normalize( const Vec2p &A )
{
	return A / Magnitude( A );
}
inline Simd_sp SqDistance( const Vec2p &A, const Vec2p &B )
{
	return SqMagnitude( B - A );
}
inline Simd_sp Distance( const Vec2p &A, const Vec2p &B )
{
	return Magnitude( B - A );
}
inline Vec2p Direction( const Vec2p &A, const Vec2p &B )
{
